...
import { User, Users, Search, AlertTriangle, Download } from "lucide-react";
import adminService from "../../services/adminService";

export default function BannedUsers({ onViewProfile, showNotification }) {
  ...
  const loadUsers = async () => {
    try {
      ...
      showNotification?.("Failed to load banned users.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExportEmails = async () => {
    try {
      ...
      if (response.success && response.users) {
        ...
        a.href = url;
        a.download = `banned_users_emails_${
                  ...
                  {users.map((user) => (
                    <tr key={user._id || user.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-3 whitespace-nowrap">
                        <button
                          onClick={(e) => handleUnbanUser(user._id, e)}
                          disabled={unbanning === user._id}
                        ...
                        >
                          ...
                          {unbanning === user._id ? "Unbanning..." : "Unban"}
                        </button>
                      </td>
                    </tr>
                  ))}
